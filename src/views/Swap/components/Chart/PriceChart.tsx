...
const PriceChart = ({
  ...
  return (
    <StyledPriceChart
      ...
      style={{border: !isMobile ? '1px solid #74EC65': '', background: 'transparent'}}
    >
      <Flex justifyContent="space-between" px="24px">
        <Flex alignItems="center">
          ...
          <IconButton variant="text" onClick={onSwitchTokens}>
            <SyncAltIcon ml="6px" color="rgba(116, 236, 101, 1)" />
          </IconButton>
          <Flex>
            <ChartButton
              aria-label={t('Basic')}
              ...
              mr="8px"
            >
              {isDesktop ? <span style={{color: '#74EC65'}}>{t('Basic')}</span> : <LineGraphIcon color="#74EC65" />}
            </ChartButton>
            <ChartButton
            ...
            >
              {isDesktop ? <span style={{color: '#74EC65'}}>{'TradingView'}</span> : <TradingViewIcon color="#74EC65" />}
            </ChartButton>
          </Flex>
        </Flex>
        {!isMobile && (
          <Flex>
            <IconButton variant="text" onClick={toggleExpanded}>
              {isChartExpanded ? <ShrinkIcon color="text" /> : <ExpandIcon color="#74EC65" />}
            </IconButton>
          </Flex>
        )}
      </Flex>
